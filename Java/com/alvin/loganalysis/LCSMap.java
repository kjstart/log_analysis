...
public class LCSMap {
	private static final AtomicInteger LCSObjectIdSeq = new AtomicInteger();
	...
	public void insert(String level, String entry, int lineId) {
		...
		if(obj == null) {
			obj = new LCSObject(seq, LCSObjectIdSeq.getAndIncrement(), level, lineId++);
			LCSObjects.add(obj);
		}else {
	...
	public LCSObject getMatch(String entry) {
		return getMatch(entry.trim().split("[\\s]+"));
	}
